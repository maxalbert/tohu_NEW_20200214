...
COVERAGE_OPTIONS ?= --cov=$(PYMODULE_NAME) --cov-append
NBVAL_OPTIONS ?= --nbval --sanitize-with=nbval_sanitize_file.cfg
...
regular-tests:
	$(PYTEST) $(COVERAGE_OPTIONS) $(PATH_TO_UNIT_TESTS)
...
notebook-tests:
	$(PYTEST) $(NBVAL_OPTIONS) $(COVERAGE_OPTIONS) $(PATH_TO_NOTEBOOK_TESTS)

validate-docs-notebooks:
	$(PYTEST) $(NBVAL_OPTIONS) $(COVERAGE_OPTIONS) $(PATH_TO_DOCS_NOTEBOOKS)
